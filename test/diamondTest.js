...
describe("DiamondTest", async function () {
	...
	before(async function () {
		...
		diamondWipeAndPaveFacet = await ethers.getContractAt("DiamondWipeAndPaveFacet", diamondAddress);
	...
	it("facets should have the right function selectors -- call to facetFunctionSelectors function", async () => {
		...
		assert.sameMembers(result, selectors);
		selectors = getSelectors(diamondWipeAndPaveFacet);
	...
	it("selectors should be associated to facets correctly -- multiple calls to facetAddress function", async () => {
		...
		assert.equal(addresses[3], await diamondLoupeFacet.facetAddress("0xf2fde38b"));
	});
	...
	it("should wipe and pave", async () => {
		const snapid = await hre.ethers.provider.send("evm_snapshot");
		const FacetNames = [
			"Test2Facet",
			"DiamondCutFacet",
			"OwnershipFacet",
		...
		tx = await diamondWipeAndPaveFacet.diamondWipeAndPave(
			facetCuts,
			ethers.constants.AddressZero,
			"0x",
			{ gasLimit: 2000000 },
		...
		receipt = await tx.wait();
		if (!receipt.status) {
			throw Error(`Diamond wipe and pave failed: ${tx.hash}`);
		}
		const selectors = getSelectors(diamondCutFacet);
		result = await diamondLoupeFacet.facetFunctionSelectors(facetCuts[1].facetAddress);
		assert.sameMembers(result, selectors);

		// Test1Facet should not exist anymore
		const test1Facet = await ethers.getContractAt("Test1Facet", diamondAddress);
		result = await diamondLoupeFacet.facetAddress(getSelectors(test1Facet)[0]);
		assert(result, ethers.utils.AddressZero);

		await hre.ethers.provider.send("evm_revert", [snapid]);
	});
